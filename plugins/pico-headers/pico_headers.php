...
/**
 * Pico plugin for Metria headers
 *
...
class Pico_Headers {
	...
	private $plugin_path;
	private $image_path = 'content/headers';
	...
	public function before_render(&$twig_vars, &$twig)
	{
		...
		include_once($this->image_path . '/' . 'headers.php');

		$twig_vars['headers'] = $this->get_files($this->image_path, $this->image_ext);
		foreach ($twig_vars['headers'] as &$p) {
	...
	// read the image info and assign the width and height
			...
			$header = $headers[$temp_array['name']];
			$temp_array['title'] = $header['title'];
			$temp_array['description'] = $header['description'];
			$temp_array['active'] = $header['active'];
			$p = $temp_array;
		}
		return;
	}
